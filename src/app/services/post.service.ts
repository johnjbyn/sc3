...
export class PostService {
  ...
  private blogData: PostResponse = {
    ...
    postCategories: [
      { id: "travel", title: "Viaggi" },
      { id: "food", title: "Cucina" },
      { id: "fashion", title: "Moda" }
    ]
  };
  ...
  getPosts(): Observable<PostResponse> {
    console.log('Loading posts from service...');
    return of(this.blogData);
  }

  aggiungiPreferito(post: Post) {
    if (!this.preferiti.find(p => p.id === post.id)) {
      this.preferiti.push(post);
    }
  }

  rimuoviPreferito(post: Post) {
    this.preferiti = this.preferiti.filter(p => p.id !== post.id);
  }
